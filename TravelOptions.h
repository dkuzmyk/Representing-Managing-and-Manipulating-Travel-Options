...
class TravelOptions{
   ...
   */
    ...
    *
    *		- the options are in increasing order of price AND
    ...
    */
    bool is_sorted()const{
        Node *str = front;                           // make a pointer to the front
        while(str->next != nullptr){                 // until the end of the linked list
            if(str->price > str->next->price)        // if the current price is more than next node's
                return false;

            else if(str->price == str->next->price){ // if price is equal, check time
                if (str->time > str->next->time)
                    return false;
            }

           // else if(str->price == str->next->price){
           //     if (str->time == str->next->time)
           //         str = str->next;
           // }

            else
                str = str->next;                      // if the fail check fails, go next
        }
    ...
    */
	...
	while(str2->next != nullptr){
        if(compare(str2, str2->next) == better)
            str2 = str2->next;
    ...
    */
    ...
    bool insert_sorted(double price, double time) {
       ...
       Node *newValue = new Node;  // when we find the value we, need create a new node
       newValue->price = price;    // assign price to the new node
       newValue->time = time;      // assign time to the new node
       newValue->next = nullptr;   // assign no next

       if(front == nullptr || newValue->price < front->price){
         newValue->next = front;
         front = newValue;
       }

       Node *curr = front;           // create a new pointer that points to the front of the LL

       // loop until the next value is more-than or equal the given value
       while(curr->next != nullptr && curr->next->price < newValue->price){
            curr = curr->next;
       }

       //cout << endl << curr->price << endl << endl;
       //cout << newValue->price << endl << endl;

       if(curr->next->price == newValue->price){
           if(curr->next->time < newValue->time){
            curr = curr->next;
            newValue->next = curr->next; // connect after equal node
            curr->next = newValue;
           }
           if(curr->next->time > newValue->time){
    ...
    bool insert_pareto_sorted(double price, double time) {
      if(!is_pareto_sorted()) return false;
       ...
       if (curr->next == nullptr){
            if (compare(newValue, curr) == worse)
                return false;
            if (compare(newValue, curr) == better){
                front = curr->next;
                delete curr;
            }
       }
       ...
       // loop and compare all the options with the new one to confirm it's not worse than any
       while(curr->next != nullptr){
            ...
            else if (compare(newValue, curr) == better){
                sizeNodes++;
                del = curr;
                curr = curr->next;
                delete(del);
